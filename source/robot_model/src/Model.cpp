  ...
  for (int i = 0; i < positions.size(); ++i) {
    ...
    if (positions(i) < this->robot_model_.lowerPositionLimit(i) + margin) {
      if (positions(i) < this->robot_model_.lowerPositionLimit(i)) {
        ...
        d = (this->robot_model_.lowerPositionLimit(i) + margin - positions(i)) / margin;
      ...
      if (this->robot_model_.upperPositionLimit(i) < positions(i)) {
        ...
        d = (positions(i) - (this->robot_model_.upperPositionLimit(i) - margin)) / margin;
        diag(i) = -2 * d * d * d + 3 * d * d;
      }
    }
  }
...
Model::inverse_kinematics(const state_representation::CartesianPose& cartesian_pose,
  ...
  const auto pinocchio_frame = this->robot_model_.frames.at(this->robot_model_.getFrameId(frame));
  const auto joint_id = pinocchio_frame.parentJoint;
