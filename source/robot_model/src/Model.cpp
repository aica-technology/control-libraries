...
Model::cwln_weighted_matrix(const state_representation::JointPositions& joint_positions, const double margin) {
  Eigen::VectorXd diag = Eigen::VectorXd::Zero(joint_positions.get_size());
...
Model::inverse_kinematics(const state_representation::CartesianPose& cartesian_pose,
  ...
  if (joint_positions.is_empty()) {
    q.set_positions(pinocchio::neutral(this->robot_model_));
  }
  auto max_retries = joint_positions ? 1 : 1;
  auto retries = 0;
  ...
  while (retries < max_retries) {
    ...
    for (unsigned int i = 0; i < parameters.max_number_of_iterations; ++i) {
      ...
      const auto iMd = this->robot_data_.oMi[joint_id].actInv(oMdes);
      ...
      auto W_c = this->cwln_weighted_matrix(q, parameters.margin);
      auto W_b = Eigen::MatrixXd::Identity(this->robot_model_.nv, this->robot_model_.nv) - W_c;
      auto J_b = J * W_b;

      auto psi = parameters.gamma * W_c * this->cwln_repulsive_potential_field(q, parameters.margin);

      pinocchio::Data::Matrix6 JJt;
      JJt.noalias() = J_b * J_b.transpose();
      JJt.diagonal().array() += parameters.damp;
      // qd.noalias() = -J.transpose() * JJt.ldlt().solve(err);
      qd.noalias() = -psi + W_b * (J_b.transpose() * JJt.ldlt().solve(err + J * psi));
      q.set_positions(pinocchio::integrate(this->robot_model_, q.get_positions(), qd * dt));

      // auto W_c = this->cwln_weighted_matrix(q, parameters.margin);
      // // auto W_b = Eigen::MatrixXd::Identity(this->robot_model_.nv, this->robot_model_.nv) - W_c;
      // auto W_b = Eigen::MatrixXd::Identity(this->robot_model_.nv, this->robot_model_.nv);
      ...
      // qd.noalias() = W_b * (J_b.transpose() * JJt.ldlt().solve(err));
