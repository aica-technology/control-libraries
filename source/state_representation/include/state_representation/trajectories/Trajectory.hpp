...
template <class StateT> class Trajectory : public State {
private:
...
public:
  ...
  void add_point(const StateT &new_point,
   ...
   */
  template <typename DurationT>
  void insert_point(const StateT &new_point,
                    const std::chrono::duration<int64_t, DurationT> &new_time,
                    int pos);
   ...
   * corresponding time
   */
  const std::pair<StateT, std::chrono::nanoseconds>
...
template <class StateT> Trajectory<StateT>::Trajectory() : State() {
...
template <class StateT>
inline void
Trajectory<StateT>::set_reference_frame(const std::string &reference_frame) {
  this->reference_frame_ = reference_frame;
}

template <class StateT>
inline const std::vector<std::string> &
Trajectory<StateT>::get_joint_names() const {
...
template <class StateT>
inline void Trajectory<StateT>::set_joint_names(
    const std::vector<std::string> &joint_names) {
  this->joint_names_ = joint_names;
}

template <class StateT> void Trajectory<StateT>::reset() {
  this->State::reset();
  this->points_.clear();
  this->times_.clear();
}
...
template <class StateT>
template <typename DurationT>
void Trajectory<StateT>::add_point(
...
void Trajectory<StateT>::insert_point(
    const StateT &new_point,
    const std::chrono::duration<int64_t, DurationT> &new_time, int pos) {
  this->set_empty(false);
...
template <class StateT> void Trajectory<StateT>::delete_point() {
  this->set_empty(false);
...
template <class StateT> void Trajectory<StateT>::clear() {
  this->points_.clear();
  this->times_.clear();
}
...
template <class StateT>
inline const std::deque<std::chrono::nanoseconds> &
Trajectory<StateT>::get_times() const {
  return this->times_;
}

template <class StateT> int Trajectory<StateT>::get_size() const {
  return this->points_.size();
}

template <class StateT>
const std::pair<StateT, std::chrono::nanoseconds>
Trajectory<StateT>::operator[](unsigned int idx) const {
  return std::make_pair(this->points_[idx], this->times_[idx]);
}

template <class StateT>
std::pair<StateT, std::chrono::nanoseconds>
Trajectory<StateT>::operator[](unsigned int idx) {
  this->set_empty(false);
  return std::make_pair(this->points_[idx], this->times_[idx]);
}
} // namespace state_representation
